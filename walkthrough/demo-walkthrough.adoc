...
+
.. _Terminal Window 2_ Run the following command to watch the payments:
+
----
...
----
+
.. _Terminal Window 1_ Run the following command to simulate an order being placed by the coolstore
+
----
...
----
+
. And remotely (to upload the binary and bake it into a new image)
+
----
...
+
.. Switch quicker to the _Developer Perspective_ to show the service spinning up
+
...
----
+
. When the compilation is done, upload the binary to make a new image
+
----
oc start-build payment --from-file target/*-runner.jar --follow
----
...
[red]#FIXME: This probably should be moved into the setup instructions#
...
----
$DEMO_HOME/scripts/template-fix.sh
----

[red]#When running in crc, you might want to wait for all the dcs to register as available before moving on to the next step.  DCs to consider at this stage are: catalog-database, datagrid-service, inventory-database, order-database, 
