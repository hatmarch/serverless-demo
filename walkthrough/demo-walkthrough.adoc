...
+
----
hey -n 50 -c 50 -m POST -D $DEMO_HOME/example/order-payload.json -T "application/json" $KN_URL
----
+
...
+
----
kn service update payment -n $dev_prj --image quay.io/mhildenb/homemade-serverless-native:initial-service-1.0 --revision-name "{{.Service}}-native"
----
+
