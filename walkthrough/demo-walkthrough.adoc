...
+
----
KN_ROUTE=<URL copied from above>
----
...
+
----
curl -i -H 'Content-Type: application/json' -X POST -d @$DEMO_HOME/example/order-payload.json $KN_ROUTE
----
+
...
== TODO: Scale and Revisions redux

. Add a native revision with `kn command`
. Show that traffic is split between the two adding some events to the orders kafka queue so that Native version picks up some
. Next blast with kafka spammer?
. Or just use hey and show distro
